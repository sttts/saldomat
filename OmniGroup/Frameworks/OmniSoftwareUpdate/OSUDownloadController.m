...
do { \
    ...
    #define NSDownloadsDirectory 15  // Added in 10.5
#endif
...
+ (void)initialize;
{
    ...
    OFVersionNumber *version10_5 = [[OFVersionNumber alloc] initWithVersionString:@"10.5"];
    HasAPIFrom10_5 = ([[OFVersionNumber userVisibleOperatingSystemVersionNumber] compareToVersionNumber:version10_5] != NSOrderedAscending);
#else
    HasAPIFrom10_5 = YES;
#endif
...
- (void)download:(NSURLDownload *)download decideDestinationWithSuggestedFilename:(NSString *)filename;
{
    ...
    // Save disk images to the user's downloads folder.  In 10.5 there is API in NSPathUtilities for this, use that if available.
    NSString *folder = nil;
    ...
    if (HasAPIFrom10_5) {
        ...
        if ([paths count] > 0) {
            ...
            if (folder && ![manager directoryExistsAtPath:folder]) {
                NSError *error = nil;
                if (![manager createPath:folder attributes:nil error:&error]) {
#ifdef DEBUG		
    ...
    if (!folder) {
        NSError *error = nil;
        folder = [[OAInternetConfig internetConfig] downloadFolderPath:&error];
        if ([NSString isEmptyString:folder]) {
#ifdef DEBUG	
            NSLog(@"Error getting download directory from Internet Config: %@", error);
#endif	    
            folder = nil;
        } else if (![manager directoryExistsAtPath:folder]) {
#ifdef DEBUG	
            NSLog(@"Cannot use nonexistent download folder of '%@'.", folder);
#endif	    
            folder = nil;
        }
    }
...
- (void)_cancel;
{
    ...
    if (!_didFinishOrFail) {
        ...
        if (![NSString isEmptyString:_destinationFile])
            [[NSFileManager defaultManager] removeFileAtPath:_destinationFile handler:nil];
    }
