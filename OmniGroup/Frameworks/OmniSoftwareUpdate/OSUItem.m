...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/branches/Staff/bungi/OSU-AutoUpdate-20070803/OmniGroup/Frameworks/OmniSoftwareUpdate/OSUChecker.m 89703 2007-08-07 22:59:50Z bungi $");
...
- initWithRSSElement:(NSXMLElement *)element error:(NSError **)outError;
{
    ...
    if (OFISEQUAL(licenseType, OSULicenseTypeExpiring)) {
        ...
        [_price release];
        _price = nil;
    } else if (OFNOTEQUAL(licenseType, OSULicenseTypeUnset) && OFNOTEQUAL(licenseType, OSULicenseTypeNone)) {
        [_price release];
