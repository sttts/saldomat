...
#ifdef DEBUG	
            NSLog(@"Error moving archive from '%@' to the trash.", archivePath);
#endif	    
	}
    }
...
{
#ifdef DEBUG
    NSLog(@"Error installing software update: %@", errorInfo);
#endif    
    return NO;
}
...
+ (NSString *)_unpackApplicationFromTarBzip2File:(NSString *)bzip2File existingVersionPath:(NSString *)existingVersionPath statusBindingPoint:(OFBindingPoint)statusBindingPoint error:(NSError **)outError;
{
    ...
    NSArray *applications = [manager directoryContentsAtPath:temporaryPath havingExtension:@"app"];
...
static BOOL NeedsAuthentication(NSString *path, uid_t pathUID, gid_t pathGID)
{
    ...
    if (pathGID != runningGID) {
        ...
        return YES; // Actually, we only need this if the group isn't in our group list.  Worry about group membership code later if anyone cares.
    }
