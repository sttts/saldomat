...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease/2008-03-20/OmniGroup/Frameworks/OmniFoundation/DataStructures.subproj/OFRationalNumber.m 98770 2008-03-17 22:25:33Z kc $");

typedef long long ofr_signed_wide;
typedef unsigned long long ofr_unsigned_wide;
#define OFR_DENOMINATOR_WIDTH (CHAR_BIT * sizeof(unsigned long))
#define OFR_DENOMINATOR_MAX ( (1LL << OFR_DENOMINATOR_WIDTH) - 1 )
...
static void ofr_reduce(ofr_signed_wide *p_, ofr_signed_wide *q_, ofr_signed_wide maxq)
{
...
static int ofr_width(ofr_signed_wide n)
{
    OBASSERT(n >= 0);
    int shift = 0;
...
static BOOL OFRationalSpecialCases(struct OFRationalNumberStruct *r, ofr_signed_wide numerator, ofr_signed_wide denominator, BOOL negative)
{
    if (numerator == 0) {
        r->numerator = 0;
        r->denominator = 0;
...
static void OFRationalFromPartsExp(struct OFRationalNumberStruct *r, ofr_signed_wide numerator, ofr_signed_wide denominator, int exponent, BOOL negative)
{
    ...
    ofr_signed_wide d = ofr_gcd_odd(numerator, denominator);
    ...
    while(exponent != 0 || numerator > OFR_DENOMINATOR_MAX || denominator > OFR_DENOMINATOR_MAX) {
        ...
        if (exponent > 0) {
            unsigned shift = MIN((unsigned)exponent, numeratorWidth - OFR_DENOMINATOR_WIDTH);
            numerator <<= shift;
            exponent -= shift;
        } else {
            unsigned shift = MIN((unsigned)-exponent, denominatorWidth - OFR_DENOMINATOR_WIDTH);
            denominator <<= shift;
            exponent += shift;
        }
        ...
        r->lop = 1;
        ofr_reduce(&numerator, &denominator, OFR_DENOMINATOR_MAX);
...
void OFRationalRound(struct OFRationalNumberStruct *n, unsigned long max_denominator)
{
    ...
    ofr_reduce(&num, &den, max_denominator);
