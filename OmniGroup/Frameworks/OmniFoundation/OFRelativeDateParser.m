...
- (NSDate *)parseDateNaturalLangauge:(NSString *)dateString withDate:(NSDate *)date timeSpecific:(BOOL *)timeSpecific useEndOfDuration:(BOOL)useEndOfDuration error:(NSError **)error;
{
	...
	if (![scanner isAtEnd]) {
		...
		NSString *name;
		NSArray *sortedKeyArray = [[relativeDateNames copyKeys] sortedArrayUsingSelector:@selector(caseInsensitiveCompare:)];
		NSEnumerator *relativeNameEnum = [[sortedKeyArray reversedArray] objectEnumerator];
		while ((name = [relativeNameEnum nextObject])) {
			...
			*timeSpecific = [[dateOffset objectAtIndex:3] boolValue];
			...
			if (codeInt != 0)
			    currentComponents = [self _addToComponents:currentComponents codeString:[[dateOffset objectAtIndex:0] intValue] codeInt:codeInt withMultiplier:[self _multiplierForModifer:[[dateOffset objectAtIndex:2] intValue]]];
		    }
		}
	    }
	    NSString *name;
	    ...
	    {
		// use a reverse sorted key array so that abbreviations come last
		NSArray *sortedKeyArray = [[specialCaseTimeNames copyKeys] sortedArrayUsingSelector:@selector(caseInsensitiveCompare:)];
		NSEnumerator *relativeNameEnum = [[sortedKeyArray reversedArray] objectEnumerator];
		while ((name = [relativeNameEnum nextObject])) {
		    NSString *match;
		    if ([scanner scanString:name intoString:&match]) {
	...
	if (number != -1) {
	    ...
	    BOOL foundCode = NO;
	    NSString *codeString;
	    NSArray *sortedKeyArray = [[codes copyKeys] sortedArrayUsingSelector:@selector(caseInsensitiveCompare:)];
