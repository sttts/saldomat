...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease/2008-03-20/OmniGroup/Frameworks/OmniFoundation/XML/OFXMLIdentifierRegistry.m 98770 2008-03-17 22:25:33Z kc $");
...
NSString *OFXMLCreateID(void)
{
    ...
    if (device) {
        ...
        if (fread(&value, 1, sizeof(value), device) == sizeof(value)) {
            static const char chars[64+1] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789-_:";

            // Encoding 64 bits 6 bits at a time yields 11 characters (64/6 == 10 + rem 4).  We'll actually encode 4 of the bits in the first character to ensure that it is a letter (which is required in the XML 'NAME' production).
    ...
    NSString *ident = [[NSString alloc] initWithFormat:@":%@", uuidString]; // Prefix with ':' to make sure it is a valid XML ID
    [uuidString release];
...
NSString *OFXMLIDFromString(NSString *str)
{
    ...
    NSString *try = [@":" stringByAppendingString:str];
    if (_OFIsValidXMLID(try))
        return try;

    return [@":" stringByAppendingString:[[str dataUsingEncoding:NSUTF8StringEncoding] unadornedLowercaseHexString]];
}
