...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease/2008-03-20/OmniGroup/Frameworks/OmniAppKit/ColorSync/OAColorProfile.m 93428 2007-10-25 16:36:11Z kc $");
...
- (NSString *)_getProfileName:(void *)aProfile;
{
    ...
    if (error != noErr) {
        ...
        if (error != noErr) {
            ...
            CMGetScriptProfileDescription((CMProfileRef)aProfile, name, &code);
    ...
    CMError error = CMCopyProfileLocalizedString((CMProfileRef)aProfile, cmProfileDescriptionTag, 0, 0, &string);
    if (error == noErr)
        return (NSString *)string;
    ...
    CMCopyProfileDescriptionString((CMProfileRef)aProfile, &string);
    return (NSString *)string; // still nil if everything errored out
#endif
}
...
- (BOOL)_rawProfileIsBuiltIn:(void *)rawProfile;
{
    ...
    if (profileLocation->locType == cmFileBasedProfile) {
#if !defined(MAC_OS_X_VERSION_10_5) || MAC_OS_X_VERSION_MIN_REQUIRED < MAC_OS_X_VERSION_10_5  // FSpMakeFSRef is deprecated on 10.5 with no replacement.  One hopes that they will only return path-based locations
        FSRef fsRef;
        ...
        // Until Apple fixes this, we'll assume it's built-in
        isBuiltIn = YES;
#endif
