...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease/2008-03-20/OmniGroup/Frameworks/OmniAppKit/OpenStepExtensions.subproj/NSColor-OAExtensions.m 93131 2007-10-20 21:33:42Z kevin $")
...
typedef struct {
    BOOL (*component)(void *container, NSString *key, float *outComponent); // Should only set the output if the key was present.  Returns YES if the key was present.
...
static BOOL _dictionaryComponentGetter(void *container, NSString *key, float *outComponent)
{
...
- (void)_addComponentsToContainer:(id)container adders:(OAColorAdders)adders omittingDefaultValues:(BOOL)omittingDefaultValues;
{
    ...
    } else if ([colorSpaceName isEqualToString:NSDeviceCMYKColorSpace] || OFISEQUAL(colorSpace, [NSColorSpace genericCMYKColorSpace])) { // There is no global name for calibrated CMYK
        NSColor *calibratedColor = [self colorUsingColorSpace:[NSColorSpace genericCMYKColorSpace]]; // don't archive device colors if at all possible

        // The -{cyan,magenta,yellow,black}Component methods are only valid for RGB colors, intuitively.
        float components[5]; // Assuming that it'll write out alpha too.
        [calibratedColor getComponents:components];
...
static BOOL _xmlCursorComponentGetter(void *container, NSString *key, float *outComponent)
{
    NSString *attribute = [(OFXMLCursor *)container attributeNamed:key];
    if (!attribute)
        return NO;
    *outComponent = [attribute floatValue];
    return YES;
}
...
static BOOL _xmlNodeAttributeDictionaryComponentGetter(void *container, NSString *key, float *outComponent)
{
