...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease/2008-03-20/OmniGroup/Frameworks/OmniAppKit/Widgets.subproj/OASearchField.m 89466 2007-08-01 23:35:13Z kc $");
...
- (void)setSearchMode:(id)newSearchMode;
{
    ...
    int selectedItemIndex = [menu indexOfItemWithRepresentedObject:newSearchMode];
    ...
    unsigned int itemIndex, itemCount = [items count];
    for (itemIndex = 0; itemIndex < itemCount; itemIndex++) {
        if ([[items objectAtIndex:itemIndex] tag] != DoNotModifyMenuItemTag)
