...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease/2008-03-20/OmniGroup/Frameworks/OmniAppKit/Widgets.subproj/OAOutlineViewEnumerator.m 68913 2005-10-03 19:36:19Z kc $")

struct _OAOutlineViewEnumeratorState {
    id item;
    int index;
    int count;
};
...
- (void) _prependItem: (id) item index: (int) index count: (int) count;
{
...
- (BOOL) _checkState
{
    ...
    id parentItem;
    struct _OAOutlineViewEnumeratorState *state;
        ...
        if (!state->index) {
            ...
            while (YES) {
                ...
                if ([_dataSource outlineView: _outlineView isItemExpandable: parent])
                    count = [_dataSource outlineView: _outlineView numberOfChildrenOfItem: parent];
