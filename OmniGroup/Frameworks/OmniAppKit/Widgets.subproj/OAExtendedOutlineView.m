...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease/2008-03-20/OmniGroup/Frameworks/OmniAppKit/Widgets.subproj/OAExtendedOutlineView.m 91093 2007-09-12 01:08:06Z wiml $")
...
- (void)editColumn:(int)columnIndex row:(int)rowIndex withEvent:(NSEvent *)theEvent select:(BOOL)select;
{
...
- (NSArray *)selectedItemsWithoutSelectedParents;
{
    ...
    result = [NSMutableArray arrayWithCapacity:[selection count]];
    for(selectedRow = [selection firstIndex]; selectedRow != NSNotFound; selectedRow = [selection indexGreaterThanIndex:selectedRow]) {
        int level = [self levelForRow:selectedRow];
        unsigned int row;
