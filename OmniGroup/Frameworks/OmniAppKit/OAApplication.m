...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease/2008-03-20/OmniGroup/Frameworks/OmniAppKit/OAApplication.m 98221 2008-03-04 21:06:19Z kc $")
...
- (NSString *)writableSupportDirectoryInDomain:(NSSearchPathDomainMask)domains withComponents:(NSString *)subdir, ...;
{
    ...
    for(pathIndex = 0; pathIndex < pathCount; pathIndex++) {
        ...
        if ([filemgr fileExistsAtPath:path isDirectory:&isDir]) {
            ...
            NSError *error = nil;
	    if ([filemgr createPath:path attributes:nil error:&error]) // only return if there is no error creating the directory
                return path;
        }
    }
    return nil;
