...
- (NSBitmapImageRep*) bitmapImageRepWithAlphaForIconFamilyElement:(OSType)elementType;
{
    ...
    if (hRawMaskData) {
        HLock( hRawMaskData );
        pRawMaskData = (void *)*hRawMaskData;
...
- (BOOL) setAsCustomIconForFile:(NSString*)path withCompatibility:(BOOL)compat
{
    ...
    if (FSGetCatalogInfo(&targetFileFSRef, kFSCatInfoFinderInfo, &catalogInfo, NULL, NULL, NULL) != noErr)
        return NO;
    ...
    if (FSSetCatalogInfo(&targetFileFSRef, kFSCatInfoFinderInfo, &catalogInfo) != noErr)
        return NO;
...
- (BOOL) setAsCustomIconForDirectory:(NSString*)path withCompatibility:(BOOL)compat
{
    ...
    FSRef targetFileFSRef, targetFolderFSRef;
    SInt16 file;
    OSErr result;
    ...
    if( ![iconrPath getFSRef:&targetFileFSRef createFileIfNecessary:NO] )
        return NO;
...
@implementation IconFamily (OAExtensions)

+ (IconFamily*) iconFamilyWithRepresentationsOfImage:(NSImage*)image;
{
    return [[[IconFamily alloc] initWithRepresentationsOfImage:image] autorelease];
}
...
- initWithRepresentationsOfImage:(NSImage*)image;
{
    ...
    // The image's reps may be NSCachedImageReps, so we convert them to NSBitmapImageReps using -initWithFocusedViewRect:.
    if ([image imageRepOfSize:ICON_SIZE_LARGE] != nil) {
        tempImage = [[NSImage alloc] initWithSize:ICON_SIZE_LARGE];
        bounds = (NSRect){NSZeroPoint, ICON_SIZE_LARGE};

        [tempImage lockFocus];
        [[image imageRepOfSize:ICON_SIZE_LARGE] drawInRect:bounds];
        iconBitmap128x128 = [[NSBitmapImageRep alloc] initWithFocusedViewRect:bounds];
    ...
    if ([image imageRepOfSize:ICON_SIZE_SMALL] != nil) {
        tempImage = [[NSImage alloc] initWithSize:ICON_SIZE_SMALL];
        bounds = (NSRect){NSZeroPoint, ICON_SIZE_SMALL};

        [tempImage lockFocus];
        [[image imageRepOfSize:ICON_SIZE_SMALL] drawInRect:bounds];
        iconBitmap32x32 = [[NSBitmapImageRep alloc] initWithFocusedViewRect:bounds];
        [tempImage unlockFocus];
        [tempImage release];
    ...
    if ([image imageRepOfSize:ICON_SIZE_TINY] != nil) {
        tempImage = [[NSImage alloc] initWithSize:ICON_SIZE_TINY];
        bounds = (NSRect){NSZeroPoint, ICON_SIZE_TINY};

        [tempImage lockFocus];
        [[image imageRepOfSize:ICON_SIZE_TINY] drawInRect:bounds];
        iconBitmap16x16 = [[NSBitmapImageRep alloc] initWithFocusedViewRect:bounds];
        [tempImage unlockFocus];
        [tempImage release];
