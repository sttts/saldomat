...
RCS_ID("$Header: svn+ssh://source.omnigroup.com/Source/svn/Omni/tags/OmniSourceRelease/2008-03-20/OmniGroup/Frameworks/OmniAppKit/Preferences.subproj/OAPreferenceController.m 93883 2007-11-02 16:59:22Z bungi $") 
...
- (IBAction)showNextClient:(id)sender;
{
    ...
    if (newIndex < [sortedClientRecords count])
        [self setCurrentClientRecord:[sortedClientRecords objectAtIndex:newIndex]];
    else
...
- (IBAction)showPreviousClient:(id)sender;
{
    ...
    newIndex = [sortedClientRecords indexOfObject:nonretained_currentClientRecord] - 1;
    if (newIndex < 0)
        [self setCurrentClientRecord:[sortedClientRecords objectAtIndex:([sortedClientRecords count] - 1)]];
    else
        [self setCurrentClientRecord:[sortedClientRecords objectAtIndex:newIndex]];
...
- (NSArray *)_categoryNames;
{
    return [[categoryNamesToClientRecordsArrays allKeys] sortedArrayUsingFunction:_OAPreferenceControllerCompareCategoryNames context:isa];
}
