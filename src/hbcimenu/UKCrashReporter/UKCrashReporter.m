	...
	NS_DURING
		...
		if( lastTimeCrashLogged )	// We have a crash log file and its mod date? Means we crashed sometime in the past.
		{
			...
			if( [lastTimeCrashReported compare: lastTimeCrashLogged] == NSOrderedAscending )
			{
				...
				// FIXME: CrashReport fuer HBCITool
				if ([appName isEqualToString:@"Saldomat hbcitool"]) {
					// Zaehler holen
					NSUserDefaults * defaults = [NSUserDefaults standardUserDefaults];
					NSDate * lastDate = [defaults objectForKey:@"hbciCrashLast"];
					int count = [[defaults objectForKey:@"hbciCrashCount"] intValue];
					
					// Crash liegt laenger als einen Tag zurueck oder hat ein Wert ueberschritten
					if ([lastDate timeIntervalSinceNow]<-24*60*60 || count>11) {
						// Crashreport ausgeben
						[[UKCrashReporter alloc] initWithLogString: currentReport];
						// Zaehler aktualisieren
						[defaults setObject:[NSDate date] forKey:@"hbciCrashLast"];
						[defaults setInteger:0 forKey:@"hbciCrashCount"];
					} else {
						count=count+1;
						[defaults setObject:[NSNumber numberWithInt:count] forKey:@"hbciCrashCount"];
					}					
				} else {
